...
RSpec.describe Tanakai::CLI do
  describe "#generate" do
    context "when generator_type is 'project'" do
      it "generates a new project with the given name" do
        expect_any_instance_of(Tanakai::CLI::Generator).to receive(:generate_project).with("foo")
    ...
    context "when generator_type is 'spider'" do
      it "generates a new spider with the given name" do
        expect_any_instance_of(Tanakai::CLI::Generator).to receive(:generate_spider).with("foo", in_project: false)
    ...
    context "when generator_type is 'schedule'" do
      it "generates a new schedule" do
        expect_any_instance_of(Tanakai::CLI::Generator).to receive(:generate_schedule)
  ...
  describe "#setup" do
    it "sets up the server with the given options" do
      builder = instance_double(Tanakai::CLI::AnsibleCommandBuilder)
      expect(Tanakai::CLI::AnsibleCommandBuilder).to receive(:new).with("user_host", {}, playbook: "setup").and_return(builder)
